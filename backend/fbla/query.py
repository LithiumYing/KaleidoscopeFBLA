...
class EventType(DjangoObjectType):
    ...
    def resolve_checkin_open(self, info):
        return self.is_checkin_open()  # type: ignore

    def resolve_checkout_open(self, info):
        return self.is_checkout_open()  # type: ignore

    def resolve_image(self, info) -> Optional[str]:
        return url_from_image_field(self.image_field) if self.image_field else None  # type: ignore
...
class StudentType(DjangoObjectType):
    ...
    def resolve_qrcode(self, info):
        return self.qr_code()  # type: ignore

    def resolve_points(self, info):
        return self.get_points()  # type: ignore

    def resolve_events_attended(self, info):
        return self.event_set.filter(elapsed=True, active_points=True).count()  # type: ignore

    def resolve_bonus_points(self, info):
        return BonusPoints.objects.filter(student__student_id=self.student_id)  # type: ignore

    def resolve_prizes(self, info):
        return Prize.objects.filter(student__student_id=self.student_id)  # type: ignore
