...
from .utils import get_school_year, image_upload_path, report_upload_path
...
class Student(models.Model):
    ...
    def qr_code(self, width=144) -> str:
        if self.student_id == self.cached_qr_code[0] and self.cached_qr_code[1] != None:
            return self.cached_qr_code[1]
        # calculate the number of modules needed based on the given width
        module_count = int(width / 12)
        # create a Micro QR code object with Model 2 and Level H error correction
        qr = qrcode.QRCode(
            version=None,
        ...
        qr.make(fit=True)
        # create a PIL image object from the QR code matrix
        qr_image = qr.make_image(fill_color="black", back_color="white")
        # resize the image to the specified width using nearest-neighbor interpolation
        ...
        # convert the PNG byte buffer to a Base64 string
    ...
    def get_points(self):
        return sum(
            [
                ...
                for event in self.event_set.all()  # type: ignore
            ]
    ...
    def update_points_fields_and_ranks(cls):
        ...
        for i, student in enumerate(cls.objects.all(), start=1):
            ...
            student.rank = (
                ...
                if previous_student and previous_student.points == student.points
                else i
            )
            student.save(update_fields=["points", "rank"])
...
class Event(models.Model):
    ...
    relapse_time = models.PositiveBigIntegerField(
        blank=True, null=True, verbose_name="How many seconds in between each event?"
    )
    ...
    def is_checkin_open(self):
        return (
            now() > (self.start_time - timedelta(minutes=5))
            and now()
            < (
                self.start_time
        ...
        try:
            if not self.elapsed:
                ...
                if not self.is_recurring:
                    self.elapsed = True
                elif self.total_count != None and self.past_count != None:
    ...
    def next_event(self):
        if (
            ...
            and self.relapse_time != None
            and self.past_count != None
        ):
            return self.start_time + timedelta(
                seconds=self.relapse_time * self.past_count
            )
        else:
            return None
